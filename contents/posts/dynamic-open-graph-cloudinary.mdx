...
4. [Design Open Graph](#design-open-graph)
...
Sharing the websites on social media is one of the most important things we can do to ensure that audience sees it. However, if we just show links, it will be difficult to gain the attention of the audience.
...
It can be seen that with the open graph, the website becomes more attractive when shared on social media.

However, if we look at the website's meta tag, we can see that the image in the meta tag is static.
...
So how can we make open graph images dynamic instead of making them one by one?

In this post, we will together create dynamic open graph images with the help of cloudinary.
...
With cloudinary, we can do URL transformations instead of using fancy editing tools. We can do this by put the transformation URL like this:
...
After that, we combine it with `header.png`, which is in the `/jagad.dev/posts/[slug]` folder.

For example, I use the slug `steam-now-playing`.
...
Since I don't want to display just a square image in the open graph, I use `hr.png` to create a diagonal line to the left of the image.
